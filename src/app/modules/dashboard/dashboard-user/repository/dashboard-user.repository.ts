...
import { T_ChangeRole, T_DashUserSchema } from "../types/dashboard-user.types";
...
class DashboardRepository {
  ...
  async createDashUser({
    ...
    const createUserProfileAndAddress = await prisma.$transaction(
      async (tx) => {
  ...
  async updateUserInfo<T extends Prisma.UserUpdateArgs>(args: T) {
    const result = await prisma.user.update(args);
    return result;
  }
}

export const dashUserRepository = new DashboardRepository();

export const dashUserRepositorys = {
  updateUserProfile,
